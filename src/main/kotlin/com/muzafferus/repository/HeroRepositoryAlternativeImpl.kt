...
class HeroRepositoryAlternativeImpl : HeroRepositoryAlternative {
    ...
    override suspend fun getAllHeroes(page: Int, limit: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = OK,
            prevPage = calculatePage(heroes, page, limit)[PREVIOUS_PAGE_KEY],
            nextPage = calculatePage(heroes, page, limit)[NEXT_PAGE_KEY],
    ...
    private fun calculatePage(
        ...
        val allHeroes = heroes.windowed(
            step = limit, size = limit, partialWindows = true
        )

        require(page <= allHeroes.size)
        val prevPage: Int? = if (page == 1) null else page - 1
        val nextPage: Int? = if (page == allHeroes.size) null else page + 1
